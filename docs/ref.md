...
**`errno` 仅在系统调用失败时被设置，调用成功时不会被清零。** 它保留上一次失败留下的值，因此只有在确认调用返回负值之后读取才有意义。

这一点决定了读写循环的分支必须**先按返回值分类，再读 `errno`**：
...
若省略中间那层、让 `n == 0` 直接落入 errno 判断，读到的是**上一次某个无关调用的残留值**，可能导致：

- 残留值恰为 `EPIPE`/`ECONNRESET` → 关闭一条完全健康的连接；
- 残留值恰为 `EINTR` → `continue` 重试 → 再次返回 0 → 无限循环，事件循环卡死。

### 3.2 返回 0 的含义：read 与 send 不对称

二者的 0 返回值**含义完全不同**，不可套用同一直觉。

| 调用 | 返回 0 的含义 |
|---|---|
| `read()` / `recv()` | **EOF——对端已关闭其写方向**（收到 FIN）。语义明确且重要，需据此置 `peer_closed_` 并返回 `IoStatus::closed`（`connections.cpp:39-42`） |
| `send()` | 仅表示"内核未接收任何字节"。**不含任何连接状态含义**，与对端是否关闭无关 |

`send()` 返回 0 的唯一正常情形是**传入长度本身为 0**。在非阻塞 TCP socket 上、长度大于 0 时，无法发送表现为返回 `-1` 且 `errno == EAGAIN`，而非返回 0。

`flush()` 的循环条件 `write_pos_ < write_buf_.size()`（`connections.cpp:72`）为严格小于，已保证传入长度恒大于 0，因此该场景下 `send()` 返回 0 属于**不应出现的状态**。代码将其归类为 `IoStatus::error`（`connections.cpp:81-83`）而非重试，理由是：既然长度非零，重试大概率仍返回 0，`continue` 会形成死循环；按异常终止连接是更安全的选择。

综上，`flush()` 中 `n == 0` 分支的作用是**双重的**：既为其后的 `errno` 判断划定有效边界（见 3.1），也避免在异常状态下陷入无限重试。

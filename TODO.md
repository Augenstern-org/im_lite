...
- [ ] **核心层错误路径三连**（同一处循环的收尾，宜一并处理）
  ...
  - [ ] `in.clear()` 语义 `core.cpp:193-195`：**位置问题已解决** —— 由「发送循环体内随循环次数
    重复执行」提到循环外，当且仅当 `send_queue` 非空时执行一次（N 次幂等清空 → 1 次，等价搬移）。
    仍未解决的是：**不可**改成解码后无条件清空。`send_queue` 为空的一种情形是半帧未齐
    （`decoder.h:45` 的 `in_buf.size() - wire_size < body_len` 返回 error → `rmp` 保持默认构造
    → 控制层 `!rmp.is_valid()` 提前 return），此时「不清空」正是拆包重组赖以工作的机制：
    下一次 EPOLLIN 把剩余字节追加到 `read_buf_` 尾部，帧凑齐后正常解码。无条件清空会丢弃半帧、
    让连接永久失步。真正的修复要等分帧状态机给出「本次消耗了多少字节」的出参（见本文件
    「分帧状态机」条），届时才能按消耗长度精确地从头部截去已消费的字节，而不是全清或不清。
